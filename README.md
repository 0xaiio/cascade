...
手动验收建议：解析一个公开视频，确认具体格式下拉同时显示分辨率与大小；解析一个小 playlist，确认条目选择、任务中心控制和进度显示正常；可选再执行一次字幕单独下载。清晰度回归验收使用 `https://youtu.be/NReDubvNjRg?si=glPVWHBZaB91s36W` 分别下载 1440p、1080p、360p，并用 `ffprobe` 或系统文件属性确认输出高度分别为 1440、1080、360。
